...
contract TokenBankV2 {
    ...
    // 实现 tokensReceived 回调接口，用于接收 transferWithCallback 的存款
    function tokenReceived(address from, uint256 amount) external {
        // 确保调用者是代币合约
        require(msg.sender == address(token), "Caller must be token contract");
